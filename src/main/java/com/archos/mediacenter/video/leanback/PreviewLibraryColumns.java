...
public final class PreviewLibraryColumns {
 ...
 public LinearLayout newRow(){LinearLayout row=new LinearLayout(context);row.setGravity(Gravity.CENTER_VERTICAL);row.setFocusable(true);row.setFocusableInTouchMode(true);row.setDescendantFocusability(ViewGroup.FOCUS_BLOCK_DESCENDANTS);android.graphics.drawable.StateListDrawable background=new android.graphics.drawable.StateListDrawable();android.graphics.drawable.GradientDrawable selected=new android.graphics.drawable.GradientDrawable();selected.setColor(PreviewAccent.alpha(context,45));selected.setStroke(dp(1),PreviewAccent.color(context));background.addState(new int[]{android.R.attr.state_focused},selected);android.graphics.drawable.GradientDrawable separator=new android.graphics.drawable.GradientDrawable();separator.setColor(0x180b1b29);android.graphics.drawable.LayerDrawable base=new android.graphics.drawable.LayerDrawable(new android.graphics.drawable.Drawable[]{separator,new android.graphics.drawable.ColorDrawable(0x244f697b)});base.setLayerHeight(1,dp(1));base.setLayerGravity(1,android.view.Gravity.BOTTOM);background.addState(new int[]{},base);row.setBackground(background);return row;}
