...
public final class PreviewLibraryColumns {
 ...
 public LinearLayout newRow(){LinearLayout row=new LinearLayout(context);row.setGravity(Gravity.CENTER_VERTICAL);row.setFocusable(true);row.setFocusableInTouchMode(true);row.setDescendantFocusability(ViewGroup.FOCUS_BLOCK_DESCENDANTS);row.setBackground(PreviewDialog.focus(context));return row;}
 ...
 private void choose(Runnable changed,int focus){List<String> labels=new ArrayList<>();Set<Integer> checked=new HashSet<>();for(int i=0;i<order.size();i++){Column c=order.get(i);labels.add(c.label+(c==Column.TITLE?" (always shown)":"")+"   ≡");if(shown.contains(c))checked.add(i);}labels.add("Reset to default");labels.add("Done");final Dialog[] menu={null};
  menu[0]=PreviewDialog.choose(context,"Choose Columns · ◀ ▶ reorder",labels.toArray(new String[0]),focus,checked,false,n->{
