...
public final class PreviewSearch extends LinearLayout {
 ...
 private static class Result {Video video;Tvshow show;String genres;Result(Video video,String genres){this.video=video;this.genres=genres;}Result(Tvshow show,String genres){this.show=show;this.genres=genres;}long id(){return show==null?video.getId():-show.getTvshowId()-2;}}
 public PreviewSearch(Activity activity,int mode,Bundle state){
  super(activity);this.activity=activity;this.mode=mode;setOrientation(VERTICAL);setPadding(dp(28),dp(12),dp(28),dp(12));setBackgroundColor(android.graphics.Color.TRANSPARENT);
  status=text("Search your library",12);status.setGravity(Gravity.END);addView(status,new LayoutParams(-1,dp(22)));
  LinearLayout columns=new LinearLayout(activity);addView(columns,new LayoutParams(-1,0,1));LinearLayout keyboardColumn=new LinearLayout(activity);keyboardColumn.setOrientation(VERTICAL);TextView heading=text("Search",27);heading.setTextColor(-1);keyboardColumn.addView(heading);TextView hint=text("Find titles in your library",12);hint.setPadding(0,dp(2),0,dp(10));keyboardColumn.addView(hint);LayoutParams keyboardSize=new LayoutParams(0,-1,.38f);keyboardSize.rightMargin=dp(24);columns.addView(keyboardColumn,keyboardSize);
  ...
  if(state!=null){selectedId=state.getLong("preview_search_selected",-1);scrollState=state.getParcelable("preview_search_scroll");restoring=selectedId!=-1;query.setText(state.getString("preview_search_query",""));}else query.requestFocus();
 }
 private void buildKeyboard(LinearLayout column){
  column.addView(new PreviewKeyboard(activity,query,()->results.requestFocus()),new LayoutParams(-1,-2));
 }
 @Override public void onWindowFocusChanged(boolean focused){super.onWindowFocusChanged(focused);if(focused)hideKeyboard();}
