...
public final class PreviewSearch extends LinearLayout {
 ...
 public PreviewSearch(Activity activity,int mode,Bundle state){
  super(activity);this.activity=activity;this.mode=mode;setOrientation(VERTICAL);setPadding(dp(28),dp(12),dp(28),dp(12));setBackgroundColor(0xff0b1b29);
 ...
 private void load(String value,int request){
  VideoLoader loader;switch(mode){case VideoSearchActivity.SEARCH_MODE_MOVIE:SearchMovieLoader movie=new SearchMovieLoader(activity);movie.setQuery(value);loader=movie;break;case VideoSearchActivity.SEARCH_MODE_EPISODE:SearchEpisodeLoader ep=new SearchEpisodeLoader(activity);ep.setQuery(value);loader=ep;break;case VideoSearchActivity.SEARCH_MODE_NON_SCRAPED:SearchNonScrapedVideoLoader file=new SearchNonScrapedVideoLoader(activity);file.setQuery(value);loader=file;break;default:SearchVideoLoader all=new SearchVideoLoader(activity);all.setQuery(value);loader=all;}
