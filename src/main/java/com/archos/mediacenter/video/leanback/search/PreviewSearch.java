...
public final class PreviewSearch extends LinearLayout {
 ...
 private void buildKeyboard(LinearLayout column){
  keyboard=new PreviewKeyboard(activity,query,()->{if(!items.isEmpty()){results.scrollToPosition(0);results.post(()->{RecyclerView.ViewHolder first=results.findViewHolderForAdapterPosition(0);if(first!=null)first.itemView.requestFocus();});}});column.addView(keyboard,new LayoutParams(-1,-2));
 }
 ...
 private void schedule(String value){if(pending!=null)main.removeCallbacks(pending);if(cancellation!=null)cancellation.cancel();int request=++generation;String search=value.trim();if(search.isEmpty()){items.clear();adapter.notifyDataSetChanged();status.setText("");return;}pending=()->load(search,request);main.postDelayed(pending,250);}
