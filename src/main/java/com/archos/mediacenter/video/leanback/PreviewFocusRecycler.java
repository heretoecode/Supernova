...
public class PreviewFocusRecycler extends RecyclerView {
 ...
 @Override public boolean dispatchKeyEvent(KeyEvent e){
  ...
  if(pending!=NO_POSITION)return true;
  ...
  if(horizontal&&(key==KeyEvent.KEYCODE_DPAD_LEFT||key==KeyEvent.KEYCODE_DPAD_RIGHT)){target+=key==KeyEvent.KEYCODE_DPAD_LEFT?-1:1;if(target<0||target>=getAdapter().getItemCount())return true;}
   ...
   if(lm instanceof GridLayoutManager){GridLayoutManager g=(GridLayoutManager)lm;int group=g.getSpanSizeLookup().getSpanGroupIndex(pos,g.getSpanCount()),span=g.getSpanSizeLookup().getSpanIndex(pos,g.getSpanCount());target=NO_POSITION;int best=Integer.MAX_VALUE;
    for(int i=pos+direction;i>=0&&i<getAdapter().getItemCount();i+=direction){int row=g.getSpanSizeLookup().getSpanGroupIndex(i,g.getSpanCount());if(row==group)continue;if(row!=group+direction)break;int distance=Math.abs(g.getSpanSizeLookup().getSpanIndex(i,g.getSpanCount())-span);if(distance<best){target=i;best=distance;}}
