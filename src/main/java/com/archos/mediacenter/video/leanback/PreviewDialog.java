...
public final class PreviewDialog {
 ...
 public static Dialog choose(Context c,String title,String[] labels,int selected,java.util.Set<Integer> checked,boolean dismissOnSelect,IntConsumer action){
  ...
  int maxHeight=c.getResources().getDisplayMetrics().heightPixels-dp(c,48);int menuHeight=Math.min(dp(c,height),maxHeight);
  ...
  scroll.setLayoutParams(new LinearLayout.LayoutParams(-1,Math.max(dp(c,30),menuHeight-dp(c,60))));w.setLayout(width,menuHeight);if(initial!=null)initial.requestFocus();return d;
 }
 ...
 public static android.graphics.drawable.Drawable menuSurface(Context c){return new android.graphics.drawable.LayerDrawable(new android.graphics.drawable.Drawable[]{surface(c,false),new PreviewFocusGlow(c)});}
