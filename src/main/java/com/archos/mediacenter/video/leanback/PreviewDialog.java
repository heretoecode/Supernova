...
public final class PreviewDialog {
 ...
 public static StateListDrawable focus(Context c){StateListDrawable s=new StateListDrawable();s.addState(new int[]{android.R.attr.state_focused},new PreviewFocusGlow(c));s.addState(new int[]{},new android.graphics.drawable.ColorDrawable(Color.TRANSPARENT));return s;}
 public static StateListDrawable buttonFocus(Context c){StateListDrawable states=focus(c);GradientDrawable neutral=new GradientDrawable();neutral.setColor(0x200b1b29);neutral.setCornerRadius(dp(c,7));neutral.setStroke(dp(c,1),0x7098aab8);StateListDrawable result=new StateListDrawable();result.addState(new int[]{android.R.attr.state_focused},new PreviewFocusGlow(c));result.addState(new int[]{},neutral);return result;}
