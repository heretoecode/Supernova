...
public final class PreviewSettings {
 ...
 public static void sidebar(PreferenceFragmentCompat fragment){
  ...
  for(String name:NAMES){PreferenceCategory target=null;for(int i=0;i<root.getPreferenceCount();i++){Preference p=root.getPreference(i);if(p instanceof PreferenceCategory&&name.contentEquals(p.getTitle())){target=(PreferenceCategory)p;break;}}if(target==null||target.getPreferenceCount()==0)continue;final PreferenceCategory category=target;
