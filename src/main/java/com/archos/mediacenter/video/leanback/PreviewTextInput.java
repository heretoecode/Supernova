...
public final class PreviewTextInput {
    public static void show(Context c,String title,String previous,int limit,Consumer<String> accept){
        ...
        LinearLayout actions=new LinearLayout(c);TextView save=button(c,"Select",()->{String value=input.getText().toString().trim();if(value.isEmpty()){input.setError("Enter a name");return;}accept.accept(value);d.dismiss();});actions.addView(save,new LinearLayout.LayoutParams(0,dp(c,38),1));actions.addView(button(c,"Cancel",d::dismiss),new LinearLayout.LayoutParams(0,dp(c,38),1));panel.addView(actions);keyboard.setLeaveDown(save::requestFocus);
