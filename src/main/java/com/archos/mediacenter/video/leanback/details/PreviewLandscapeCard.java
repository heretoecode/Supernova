...
final class PreviewLandscapeCard extends LinearLayout {
    ...
    PreviewLandscapeCard(Context c){super(c);setOrientation(VERTICAL);setFocusable(true);setFocusableInTouchMode(true);setDescendantFocusability(FOCUS_BLOCK_DESCENDANTS);setClipChildren(false);setClipToPadding(false);setBackground(PreviewDialog.focus(c));
        FrameLayout art=new FrameLayout(c);android.graphics.drawable.GradientDrawable clip=new android.graphics.drawable.GradientDrawable();clip.setColor(0xff101c28);clip.setCornerRadius(dp(6));art.setBackground(clip);art.setClipToOutline(true);addView(art,new LayoutParams(-1,dp(102)));image=new ImageView(c);image.setScaleType(ImageView.ScaleType.CENTER_CROP);art.addView(image,new FrameLayout.LayoutParams(-1,-1));
