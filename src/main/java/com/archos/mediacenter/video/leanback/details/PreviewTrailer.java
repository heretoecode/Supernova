...
public final class PreviewTrailer {
    public static void show(Activity activity,ScraperTrailer trailer){
        ...
        LinearLayout bar=new LinearLayout(activity);bar.setGravity(Gravity.CENTER_VERTICAL);bar.setPadding(20,8,20,8);layout.addView(bar,new LinearLayout.LayoutParams(-1,60));
        TextView title=new TextView(activity);title.setText(trailer.mName);title.setTextColor(0xffffffff);title.setSingleLine(true);title.setEllipsize(android.text.TextUtils.TruncateAt.END);bar.addView(title,new LinearLayout.LayoutParams(0,-2,1));
        for(String label:new String[]{"Open YouTube","Close"}){TextView button=new TextView(activity);button.setText(label);button.setTextColor(0xffffffff);button.setPadding(20,8,20,8);button.setFocusable(true);button.setBackgroundResource(com.archos.mediacenter.video.R.drawable.mark_hud_focus);button.setOnClickListener(v->{dialog.dismiss();if(label.equals("Open YouTube"))external(activity,fallback);});bar.addView(button);}
