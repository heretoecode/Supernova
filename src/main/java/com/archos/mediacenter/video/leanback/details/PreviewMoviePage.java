...
public final class PreviewMoviePage extends ScrollView {
    ...
    public PreviewMoviePage(Context c,Supplier<ObjectAdapter> actions,Consumer<Action> action,Runnable nativeDetails,Consumer<Uri> artwork){
        ...
        plot=text("",13);plot.setMaxLines(3);plot.setEllipsize(android.text.TextUtils.TruncateAt.END);hero.addView(plot,new LinearLayout.LayoutParams(Math.min(dp(450),getResources().getDisplayMetrics().widthPixels-dp(250)),dp(48)));
        LinearLayout buttons=new LinearLayout(c);buttons.setPadding(0,dp(12),0,dp(10));hero.addView(buttons);
        play=button("Play",this::play);buttons.addView(play);providerActions=new LinearLayout(c);HorizontalScrollView services=new HorizontalScrollView(c);services.setHorizontalScrollBarEnabled(false);services.addView(providerActions);hero.addView(services,new LinearLayout.LayoutParams(-1,-2));
    ...
    private TextView button(String s,Runnable run){TextView t=text(s,14);t.setTag("action:"+s);PreviewIcon.apply(t,s,16);t.setGravity(Gravity.CENTER);t.setPadding(dp(14),dp(9),dp(14),dp(9));t.setFocusable(true);t.setFocusableInTouchMode(true);t.setBackground(PreviewDialog.focus(getContext()));t.setOnFocusChangeListener((v,f)->{t.setTextColor(f?Color.WHITE:0xffbdd0de);t.setShadowLayer(f?dp(5):0,0,0,PreviewAccent.color(getContext()));});t.setOnClickListener(v->run.run());return t;}
    ...
    private void person(LinearLayout people,String name,String role){
        LinearLayout card=new LinearLayout(getContext());card.setOrientation(android.widget.LinearLayout.HORIZONTAL);card.setGravity(Gravity.CENTER_VERTICAL);card.setBackgroundColor(0x50101e2b);card.setForeground(PreviewDialog.focus(getContext()));card.setFocusable(true);card.setFocusableInTouchMode(true);card.setPadding(dp(6),dp(6),dp(6),dp(8));
        ...
        TextView label=text(name,12);label.setMaxLines(2);label.setEllipsize(android.text.TextUtils.TruncateAt.END);words.addView(label);TextView detail=text(role==null?"":role,10);detail.setMaxLines(1);detail.setTextColor(0xffa4b6c7);words.addView(detail);card.setContentDescription(name+" "+safe(role));LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(dp(142),dp(58));lp.rightMargin=dp(10);people.addView(card,lp);card.setTag("person:"+name+":"+safe(role));rowKeys(card,people);
    }
    ...
    private void moreInfo(){
        ...
        dialog.setContentView(scroll);dialog.setOnDismissListener(d->{if(previous!=null)previous.requestFocus();});dialog.show();android.view.Window window=dialog.getWindow();window.setBackgroundDrawableResource(android.R.color.transparent);window.addFlags(android.view.WindowManager.LayoutParams.FLAG_DIM_BEHIND);window.setDimAmount(.72f);int width=Math.min(dp(760),getResources().getDisplayMetrics().widthPixels-dp(64));int maximum=getResources().getDisplayMetrics().heightPixels-dp(72);scroll.measure(View.MeasureSpec.makeMeasureSpec(width,View.MeasureSpec.EXACTLY),View.MeasureSpec.makeMeasureSpec(maximum,View.MeasureSpec.AT_MOST));window.setLayout(width,Math.min(maximum,scroll.getMeasuredHeight()));window.setGravity(Gravity.CENTER);scroll.requestFocus();
    }
