...
public final class PreviewMoviePage extends ScrollView {
    ...
    public void setTags(BaseTags value,List<ScraperTrailer> videos,List<ScraperImage> backdrops){tags=value;trailerList=videos==null?Collections.emptyList():videos;
        ...
        cast.removeAllViews();HorizontalScrollView scroll=new HorizontalScrollView(getContext());scroll.setHorizontalScrollBarEnabled(false);LinearLayout people=new LinearLayout(getContext());scroll.addView(people);cast.addView(scroll);
    ...
    private void renderDetails(){if(movie==null&&show==null)return;details.removeAllViews();
        ...
        if(movie!=null){panel(row,"Source",source(movie.getFileUri()),"File",movie.getFilenameNonCryptic());panel(row,"Video",movie.getCalculatedVideoFormat(),"Audio",movie.getCalculatedBestAudioFormat());}
    }
    ...
    private void person(LinearLayout people,String name,String role){
        LinearLayout card=new LinearLayout(getContext());card.setOrientation(1);card.setBackground(bg(false));card.setFocusable(true);card.setFocusableInTouchMode(true);card.setOnFocusChangeListener((v,f)->v.setBackground(bg(f)));card.setPadding(dp(6),dp(6),dp(6),dp(8));
        // The existing scraper stores names/roles but no portrait URLs. Use the real Nova asset.
        ImageView portrait=new ImageView(getContext());portrait.setImageResource(com.archos.mediacenter.video.R.drawable.nova_banner);portrait.setScaleType(ImageView.ScaleType.FIT_CENTER);card.addView(portrait,new LinearLayout.LayoutParams(-1,dp(90)));
    ...
    public void setSeason(int number,List<Episode> values){
        ...
        for(Map.Entry<Integer,List<Episode>> season:seasons.entrySet()){
            episodes.addView(text(season.getKey()==0?"Specials":"Season "+season.getKey(),16));HorizontalScrollView scroll=new HorizontalScrollView(getContext());LinearLayout row=new LinearLayout(getContext());scroll.addView(row);episodes.addView(scroll);
            for(Episode e:season.getValue()){Presenter.ViewHolder h=presenter.onCreateViewHolder(row);presenter.onBindViewHolder(h,e);h.view.setTag("episode:"+e.getId());h.view.setContentDescription("S"+e.getSeasonNumber()+" E"+e.getEpisodeNumber()+" · "+e.getEpisodeName());LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(dp(172),dp(105));lp.setMargins(0,dp(6),dp(12),dp(12));row.addView(h.view,lp);h.view.setOnClickListener(v->new VideoViewClickedListener((Activity)getContext()).onItemClicked(h,e,null,null));if(old!=null&&old.equals(h.view.getTag()))h.view.requestFocus();}
        }
        scrollTo(0,y);
    }
    ...
    private void more(){ObjectAdapter adapter=actions.get();if(adapter==null)return;List<Action> items=new ArrayList<>();List<String> labels=new ArrayList<>();
        for(int i=0;i<adapter.size();i++){Object value=adapter.get(i);if(value instanceof Action){Action a=(Action)value;if(show!=null&&a.getId()==com.archos.mediacenter.video.leanback.tvshow.TvshowActionAdapter.ACTION_MORE_DETAILS)continue;items.add(a);labels.add(String.valueOf(a.getLabel1())+(a.getLabel2()==null?"":" — "+a.getLabel2()));}}
        if(show==null)labels.add("File, subtitles and artwork");PreviewDialog.choose(getContext(),"More",labels.toArray(new String[0]),-1,n->{if(n==items.size())nativeDetails.run();else action.accept(items.get(n));});
    }
