...
public final class PreviewMoviePage extends ScrollView {
    ...
    public void setSeason(int number,List<Episode> values){
        ...
        if(show!=null){List<Entry> all=new ArrayList<>();for(List<Episode> group:seasons.values())for(Episode ep:group){Entry entry=new Entry(ep,0,show.getTvshowId(),"");if(show.getShowTags()!=null)entry.onlineId=show.getShowTags().getOnlineId();all.add(entry);}PreviewSeriesJourney.Selection selected=PreviewSeriesJourney.select(getContext(),all);play.setText(selected.episode!=null&&PreviewSeriesJourney.resumable((Video)selected.episode.media)?"Resume":"Play");}
        scrollTo(0,y);
    }
