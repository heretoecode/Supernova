...
public final class PreviewSeriesJourney {
    private static SharedPreferences prefs(Context c) { return c.getSharedPreferences("preview_series_journey41",Context.MODE_PRIVATE); }
    ...
    private static String key(List<Entry> entries) {
        ...
        return online>0?"show:"+online+":":"local:"+first.show+":";
    }
    ...
    public static void record(Context c,VideoDbInfo info,boolean completed,long viewedMs) {
        ...
        SharedPreferences p=prefs(c);String key="show:"+info.scraperShowId+":";SharedPreferences.Editor edit=p.edit();
        edit.putBoolean("attempt:"+info.id,true);
    ...
    public static void explicitlyWatched(Context c,Video video){if(video instanceof Episode&&!PrivateMode.isActive())prefs(c).edit().putLong("mark:"+video.getId(),System.currentTimeMillis()/1000L).apply();}
    ...
    public static Selection select(Context c,List<Entry> source){
        ...
        for(Entry e:episodes){long marked=p.getLong("mark:"+((Video)e.media).getId(),0);if(marked>time){Episode ep=(Episode)e.media;season=ep.getSeasonNumber();number=ep.getEpisodeNumber();complete=true;time=marked;}}
        if(season<0){
            // One-time migration of 4.0 progress, excluding files merely sampled in 4.1.
            Entry seed=episodes.stream().filter(e->resumable((Video)e.media)&&!p.getBoolean("attempt:"+((Video)e.media).getId(),false)).max(Comparator.comparingLong(e->((Video)e.media).getLastPlayed())).orElse(null);
        ...
        if(started){
            ...
            if(!PrivateMode.isActive()){SharedPreferences.Editor edit=p.edit();put(edit,key,season,number,complete,time);edit.apply();}
        }else{
